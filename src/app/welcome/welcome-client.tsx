...
function StyleMoodBoard({ src, name }: { src: string; name: string }) {
  // `key` forces remount on src change so the CSS fade-in animation re-runs
  // — avoids the synchronous-setState-in-effect anti-pattern of a manual
  // crossfade.
  return (
...
export function WelcomeClient({ signedIn }: { signedIn: boolean }) {
  ...
  const finishOnboarding = (finalStylePrefs: Record<string, string> = stylePrefs) => {
    try {
      ...
      // sessionStorage unavailable (private mode etc.) — just proceed
    }
    if (signedIn) {
      router.push("/match-quiz");
    } else {
      router.push("/sign-up?redirect_url=/match-quiz");
    }
  };
