...
export default function StylistDashboard({
  ...
  const rightPanel = selected ? (
    <div className="flex-1 flex flex-col min-w-0 bg-muted/30">
      ...
      {/* Messages + composer — backend-wired chat panel. Replaces the prior
          local-state mock that lost messages on refresh. Uses the shared
          `useChat` hook (DB bootstrap + Twilio realtime) and posts via
          /api/sessions/[id]/messages so the inline DB mirror covers
          webhook delivery loss. */}
      <DashboardChatPanel sessionId={selected.id} stylistClerkId={stylistClerkId} />
    </div>
  ) : (
