...
export interface FreestyleItem {
  src: string;
  // Normalized 0-1 coordinates relative to canvas (top-left origin)
  x: number;
  y: number;
  w: number;
  h: number;
...
/**
 * Freestyle canvas — drag to position, drag corner handle to resize.
 * Coordinates are stored normalized (0-1) so the layout scales with the canvas.
 */
  ...
  useEffect(() => {
    const handleMove = (e: PointerEvent) => {
      ...
      const next = items.map((it, n) => {
        ...
        // resize — keep aspect by using max delta
...
/**
 * Generate a default freestyle layout from a list of image srcs.
 * Distributes them in a loose grid with slight variation.
 */
