...
test.describe("styleboard shop workspace", () => {
  ...
  test("Looks like canvas + Find pieces are disabled without canvas items", async ({
    page,
  }) => {
    await signInStylist(page);
    await page.goto(`/stylist/sessions/${sessionId}/styleboards/new`);
    ...
    await expect(
      page.getByRole("button", { name: /Looks like canvas/i }),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", { name: /Find pieces for this look/i }),
    ).toBeDisabled();
  });

  test("Suit pair dialog opens + sends colorFamily", async ({ page }) => {
    await signInStylist(page);

    let suitPairBody: Record<string, unknown> | null = null;
    await page.route(
      `**/api/stylist/sessions/${sessionId}/shop-inventory/suit-pairs`,
      async (route) => {
        suitPairBody = route.request().postDataJSON?.() as
          | Record<string, unknown>
          | null;
        // Stub a deterministic response so the modal renders without
        // depending on staging suit_pairs data.
        await route.fulfill({
          status: 200,
    ...
    await page.goto(`/stylist/sessions/${sessionId}/styleboards/new`);
    await page.getByRole("button", { name: /^Suit pair$/i }).click();

    await expect(page.getByRole("dialog")).toBeVisible();
    await page.getByRole("button", { name: /^Navy$/ }).click();
    await page.getByRole("button", { name: /Find pairs/ }).click();

    await page.waitForRequest((req) =>
      req.url().includes("/shop-inventory/suit-pairs"),
    );
    expect(suitPairBody).toBeTruthy();
    expect((suitPairBody as Record<string, unknown>).colorFamily).toBe("navy");
  });
});
