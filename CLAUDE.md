...
- [x] Phase 11: Polish & Launch — PR#36 merged 2026-04-23 (`56aed71`). Klaviyo events, EasyPost carrier integration, SEO metadata, observability dashboards, DB indexes.
- [~] Funnel redesign Wave A — branch `funnel-redesign`, in progress. smart-spark-craft (Loveable) advanced 975 commits between 2026-04-08 and 2026-04-19 after the Phase 10 port baseline. Wave A catches the conversion path up: new public `/welcome` 4-step onboarding (needs / department / body / style mood-board vote), `/matches` refactored from 3-card grid to single-hero "Perfect Match" layout (portfolio carousel from real `profileBoards.photos`, alternates row, inline How-It-Works carousel), 7 style mood-board assets (`public/img/style-{minimal,feminine,chic,classic,bohemian,street,sexy}.png`), and a `tests/e2e/funnel-redesign.spec.ts` (3 specs, all green). `/welcome` added to `proxy.ts` public routes (anonymous visitors flow through onboarding before sign-up redirect). Locked-out copy preserved: "Seasonal Capsule" → "Seasonal Refresh". Architectural deviations from Loveable kept: Clerk auth (no SignUpModal), `lib/plans.ts` price source, dropped manual `setState`-in-effect crossfade in favour of `key`-driven CSS animation. Deferred to subsequent waves: `/style-quiz` deep questionnaire (STYLE_PREFERENCE schema diverges from Loveable's 7 styles — needs founder call), `/checkout` (would replace working `/cart` → Stripe Hosted Checkout — needs founder call), `/gift-cards`, `/reviews`, `/board/[id]` SharedBoard, `/stylists/[id]/reviews`, plus the page-level redesigns of Index / Settings / Pricing / Feed / Stylists / LuxPackage / HowItWorks / StylingRoom / StylistProfile.
