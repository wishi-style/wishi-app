...
- **Stylist `actionLabel` vocabulary diverges from Loveable on purpose.** Loveable's `Start styling` / `View session` / `Awaiting approval` / `View summary` were no-op selectors that broke once real PendingAction states started flowing in production. Replaced by `deriveDashboardAction` in `src/lib/sessions/stylist-dashboard.ts`, which returns `{ label, href, kind }` so every CTA navigates to a real destination. Six labels: `Create Moodboard` (no moodboard sent) → `/moodboards/new`, `Create Look` (looks remain) → `/styleboards/new`, `Review Restyle` (`PENDING_RESTYLE` with `boardId`) → `/styleboards/new?parentBoardId=<X>`, `Approve End` (`endRequestedAt` set, `kind: "approve-end"`) → fires `approveEndSession` in place, `View Summary` (`COMPLETED`/`CANCELLED`) → `/stylist/dashboard?session=<id>`, `Open Chat` (everything else) → `/stylist/dashboard?session=<id>`. Order matters — terminal/blocking states (completed, end-requested) win over progress states. The card's quota-met dual-CTA pairs the primary (whatever `actionLabel` resolved to) with `End session`. Pinned by `tests/stylist-dashboard-adapters.test.ts` + `tests/e2e/stylist-action-vocabulary.spec.ts`.
- **`ClientProfileView` adapter quirks.** Free-text quiz answers (`BodyProfile.bodyIssues`, `SpecificPreference.denimFit`, `jewelryPreference`) split on `,;\n` to fill Loveable's `string[]` chip arrays. `StyleProfile.comfortZoneLevel` (Int 1-10 from the RANGE quiz) buckets to phrases: `1-3 → "Stay close"`, `4-7 → "A little outside"`, `8-10 → "Push my boundaries"`. `UserSocialLink.url` extracts the trailing path segment as a handle (`@feizhen.style` for instagram, bare `feizhen_d` for pinterest/facebook). `bodyProfile.bodyIssues` is surfaced only as `bodyIssueNotes` — the chip array stays empty until the schema grows a separate chip-list column.
