...
**Audit-driven items still deferred** (tracked in `WISHI-REBUILD-PLAN.md`): inline interactive Love/Revise/Not-my-style feedback inside the StyleBoard chat bubble, stylist top-nav chrome (Calendar/Bell/Settings/dropdown), dashboard "Item Recommendation" form's local-state-only `handleSendItem`, MoodBoardCreator "Client info" full-page nav vs side sheet, `/stylist/sessions` list re-skin or redirect.
