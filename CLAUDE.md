...
**Client pixel-parity follow-up (this PR):** `CLIENT-PIXEL-PARITY-TASK.md` re-audit on `smart-spark-craft@8dfd181` (unchanged from the prior 2026-04-28 audit). Fresh re-walk found zero UNINT items remaining after PRs #41/#67/#69/#71/#72/#73/#76/#77/#78/#80/#81. New work landed: `/discover` ported from Loveable's `StylistsDiscover.tsx` (`src/app/discover/page.tsx`) wired to live `prisma.stylistProfile.findMany` + `?q=` search; `tests/visual/client.spec.ts` + `playwright.visual-client.config.ts` adding empty-state baselines for authed surfaces (`/sessions`, `/cart`, `/orders`, `/favorites`, `/profile`, `/settings`); `tests/visual/marketing.spec.ts` extended with five new public routes (`/discover`, `/reviews`, `/gift-cards`, `/match-quiz`, `/stylist-match`); `.github/workflows/visual-regression.yml` re-introduced after PR #61's removal — `workflow_dispatch update_snapshots=true` uploads regenerated linux baselines as a `linux-baselines` artifact, sidestepping the linux-bootstrap problem that killed the first iteration. Audit doc at `wishi-style/CLIENT-PARITY-AUDIT.md` rewritten with the new `BACKEND-GAP` column.
