...
- **Proxy onboarding gate (Phase 6):** `src/proxy.ts` redirects stylists mid-wizard away from `/stylist/*` (and `/api/stylist/*`) to `/onboarding/[step]` unless `onboardingStatus ∈ {AWAITING_ELIGIBILITY, ELIGIBLE}`. The wizard's own routes + profile-boards + uploads are exempt so the wizard can call back into itself. E2E mode short-circuits the gate via the existing `E2E_CLERK_ID_COOKIE` early-return.
- **Internal workers (Phase 6):** `src/workers/*.ts` contain the logic; `src/app/api/workers/<name>/route.ts` are thin HTTP endpoints guarded by `src/lib/workers/auth.ts` (shared-secret `x-worker-secret` header matched against `WORKER_SHARED_SECRET` env). EventBridge Scheduler (module `infra/modules/scheduler/`) fires them — waitlist-notify hourly, payout-reconcile Mondays 06:00 UTC. Auth fails closed when the env var is unset.
- **Tip flow (Phase 6):** `/sessions/[id]/end-session` client component uses `@stripe/stripe-js` + `@stripe/react-stripe-js` `PaymentElement`. Server Action `submitEndSessionFeedback` writes `Session.rating`/`reviewText`/`ratedAt` immediately (not payment-gated) and creates a Stripe PaymentIntent with `metadata.purpose="tip"`. The durable write for `Session.tipInCents` + `Payment(type=TIP)` happens in the `payment_intent.succeeded` webhook — the Server Action is optimistic.
