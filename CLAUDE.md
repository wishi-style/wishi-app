...
- **Stylist `actionLabel` vocabulary.** Mirrors Loveable verbatim: `Create Moodboard` / `Create Look` / `Review Restyle Request` / `View session` (lowercase s) / `Awaiting approval` / `View summary` / `Start styling`. Card and chat-header click handlers ONLY navigate for `Create Moodboard` and `Create Look`; every other label is a no-op selector. The literal `View Session` (capital S) is the Pass-1+2 drift bug — pinned by `tests/stylist-dashboard-adapters.test.ts`.
