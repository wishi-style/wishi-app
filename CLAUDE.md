...
- **Unicart commerce (assisted shopping):** `src/lib/cart/cart.service.ts` owns the session-scoped cart (upsert on `[userId, inventoryProductId, sessionId]` — re-adds increment quantity). **No allow-list gate** — every inventory item with a retailer link is Wishi-shoppable. The human fulfiller buys from each retailer on the user's behalf using the shipping info captured at checkout, so users receive retailer shipping emails directly. `src/lib/payments/direct-sale.service.ts` (Hosted Checkout) and `direct-sale-elements.service.ts` (Stripe Elements at `/checkout`) both run `resolveLineItems` to validate cart rows and snapshot the retailer name onto each `OrderItem.retailerName` at order creation. `automatic_tax.enabled=true` (Stripe Tax is the only tax authority — we never recompute) and a fixed shipping rate ($10 standard, $0 for active Lux sessions). The checkout pre-creates `Order(status=PENDING)` carrying the cart snapshot; the webhook flips PENDING → ORDERED via conditional `updateMany` keyed on `Order.stripeCheckoutSessionId`. **Per-item fulfillment state lives on `OrderItem.status`** (`OrderItemStatus`: PENDING → PURCHASED → UNFULFILLABLE → RETURN_REQUESTED → RETURNED) — `Order.status` is the roll-up (PENDING → ORDERED → COMPLETED). The legacy SHIPPED/ARRIVED/RETURN_IN_PROCESS/RETURNED `OrderStatus` values stay valid for in-flight Phase 11 orders; new orders go through the per-item state machine. `src/lib/orders/admin-orders.service.ts` owns the fulfillment service; the per-item `transitionOrderItemStatus` (TBD — service+UI land in follow-up commits on the `unicart-remove-direct-sale-gate` branch) drives PURCHASED (captures retailer order ref) and UNFULFILLABLE (captures reason + fires a per-line partial Stripe refund; shipping is refunded only when *every* item in an order fails). Returns flow direct-to-retailer: the user submits "I returned this to {Retailer} — mirror my refund" with a receipt reference; admin verifies and Stripe-refunds the line. `ClosetItem` auto-create fires on `OrderItem.status = PURCHASED` (the new flow may never reach the legacy `Order.status=ARRIVED` because retailer shipping/delivery emails reach the user directly, not Wishi — TBD trigger flip is part of the follow-up commits). Admin refunds wrap Stripe `refunds.create` with an `idempotencyKey` keyed on `(orderId, prevRefundedInCents, amountInCents)`; per-OrderItem refunds use `unfulfillable_refund:{orderItemId}` / `return_refund:{orderItemId}`. The manual closet upload path in `lib/boards/closet.service.ts` MUST NEVER accept `sourceOrderItemId` — that field is reserved for `closet/auto-create.ts` and admin tooling.
