...
4. **`npm run test:visual`** — Playwright visual regression against committed baselines (marketing + new public client routes — anonymous). Config: `playwright.visual.config.ts`. Two projects: `desktop-chrome` (1280×800) + `mobile-chrome` (Pixel 7). Baselines under `tests/visual/marketing.spec.ts-snapshots/` with per-platform suffix (`-darwin`, `-linux`). `npm run test:visual:update` regenerates. 0.5% delta budget. Enforced in CI via `.github/workflows/visual-regression.yml` (re-introduced after the client pixel-parity sweep — bootstrap linux baselines via `workflow_dispatch update_snapshots=true`, download the `linux-baselines` artifact, commit). Sibling configs `playwright.visual-stylist.config.ts` (`npm run test:visual:stylist`) and `playwright.visual-client.config.ts` (`npm run test:visual:client`) cover authed surfaces — local-only because they need DB seeding.
...
- **Stylist frontend port (Phase 12):** Loveable `wishi-reimagined` becomes the 1:1 design contract for every stylist-role surface (client-side was Phase 10 via `smart-spark-craft`). Dashboard (`src/app/(stylist)/stylist/dashboard/*`) reads from `src/lib/sessions/stylist-dashboard.ts` — a view-model mapper over `Session + SessionPendingAction + Message + client.loyaltyTier` that returns priority-ranked DashboardSession rows. Session workspace (`src/components/stylist/workspace.tsx`) wraps the existing Twilio-backed ChatWindow in Loveable chrome (left rail with client avatar + vertical tabs + Build Moodboard / Build Styleboard / End Session action footer). **Canvas styleboard model** — `BoardItem.x/y/zIndex` (nullable) stores the LookCreator drag-drop position; `Board.description/tags` (nullable) carry the save-dialog metadata. All fields are additive — moodboards + legacy styleboards stay intact; the client-side viewer (`StyleBoard.tsx` from Phase 10) remains a columns-2 mosaic and ignores x/y. `sendStyleboard` now enforces `items.length >= 3` and persists `{ title, description, tags }` inside the same atomic transaction that flips `sentAt`. Background removal is deferred to Phase 7 — the LookCreator eraser button toasts "coming soon" so the UI is feature-complete visually without shipping a 15MB ML bundle.
...
- [x] Phase 11: Polish & Launch — PR#36 merged 2026-04-23 (`56aed71`). Klaviyo transactional emails (Events API fan-out, 5 new NotificationEvents covering orders + subscription retry) + EasyPost tracker webhooks driving SHIPPED → ARRIVED automatically + root/route-group error boundaries + SEO (sitemap, robots, JSON-LD, dynamic OG images) + k6 load-test harness + architecture/runbook/ADR docs + CloudWatch launch dashboard with tuned alarms + hot-path DB indexes (orders.tracking_number, sessions composites, boards feed) + visual-regression GH Actions workflow + security audit (SECURITY.md + response headers) + Stripe CLI gift-card webhook-chain e2e harness.
- [x] Phase 12: Stylist App Frontend Port — PR#58 merged 2026-04-24 (`947eda4`). Loveable `wishi-reimagined@13df7f5` is the 1:1 design contract for every stylist-role surface. Foundation (stylist view-model, Bodoni Moda font, 18 shadcn primitives, 7 new deps) + StylistDashboard (1028 LOC, wired to real `Session + PendingAction + Message + LoyaltyTier` via `src/lib/sessions/stylist-dashboard.ts`; right-rail chat wired to real Twilio-mirrored `Message` rows + POST send) + Session workspace (`src/components/stylist/workspace.tsx` — Loveable chrome wrapping real Twilio ChatWindow + Cart/Closet/Inventory tabs) + canvas schema migration (`BoardItem.x/y/zIndex` + flip/crop fields + `Board.description/tags`, `StylistPrivateNote` model, all additive) + MoodBoardCreator (Pinterest-style inspiration grid + asymmetric canvas + AI-stub note dialog) + LookCreator drag-drop canvas composer (3–12 items, x/y% positioning, save dialog writing `{ title, description, tags[] }`, advanced filter panel with retailers/colors/sizes/price/in-stock/sale, crop + flip H/V tools, canvas-size toggle 1/2/3 keys, inventory favorites heart, Previous Looks tab with stylist-level + client-level scope toggle) + full ClientProfileView resolver + ClientDetailPanel wiring (StyleProfile + BodyProfile + BodySize + color/fabric/pattern/specific prefs + budgets + social + photos + favorite boards + session count + stylist private note) + clients roster/sessions list/detail. Typecheck + lint clean. `npm test` 271/302 passing, 0 failing, 31 skipped. Phase 12 Playwright: 4 authed e2e specs (`tests/e2e/stylist-frontend-port.spec.ts`) + 8 visual baselines (4 routes × 2 viewports) under dedicated `playwright.visual-stylist.config.ts` harness + `scripts/e2e-stylist-walkthrough.ts` (10/10 checks green). **Still deferred:** real HuggingFace background removal (Phase 7 server-side worker), real LLM for moodboard note (Phase 7), preorder/final-sale filter chips (upstream tastegraph gap — `FilterValuesResponse` doesn't expose them yet), Linux visual baselines for stylist routes (darwin baselines committed; `npm run test:visual:stylist` runs locally — linux capture needs a Docker run and is tracked alongside the pre-existing Phase 10 linux-baselines gap noted in `WISHI-REBUILD-PLAN.md`). Phase 7 (AI) remains the only other open phase.
...
- **Wave B — Page design refresh** (9/9 complete): homepage (#45), how-it-works (#42), lux (#43), pricing (#44), feed (#47), stylists (#46), settings card-grid (#49), StylingRoom shell (#50 SessionHeaderBar + #67 full Loveable left rail + chat primitives), stylist-profile hero+Meet+Trust+sticky-CTA (#51).
...
**StylingRoom shell parity (PR #67 merged 2026-04-28 at `a98b069`):** A 2026-04-28 audit (`wishi-style/CLIENT-PARITY-AUDIT.md` + `STYLIST-PARITY-AUDIT.md`) found the chat primitives + client shell had drifted from Loveable despite Wave B #50 (slim SessionHeaderBar only) and Phase 12 #58. PR #67 closes the bulk: ChatInput rebuilt as a `rounded-full` pill + Plus-popover (Add a file / Inspiration library / Open camera) + Mic + circular Send + "Message {firstName}" placeholder; bubbles re-skinned to `rounded-[22px]` + design tokens (`hsl(var(--user-bubble))`); inline `<MoodBoardMessage>` (3-col masonry) + `<StyleBoardMessage>` (columns-2 mosaic) replace the Phase 10 link-card; real `<SingleItemCard>` with Add-to-Cart hitting `POST /api/cart`; closed-session "Book a new session" link replaces the disabled composer; new `GET /api/styleboards/[id]/preview` resolves item thumbnails server-side. Client `session/workspace.tsx` rebuilt as the Loveable 256px left rail (back arrow + stylist + plan badge + Looks-delivered progress + vertical tabs + bottom-anchored Buy Looks + Upgrade Plan), dropping the horizontal tab strip, the rebuild-only "Moodboard" tab, and the right-rail SessionSidebar. Stylist `stylist/workspace.tsx` keeps its Phase 12 Loveable chrome; chat-body upgrade flows through `<ChatWindow>` with a new `hideHeader` prop. **Dashboard parity P0 batch (PR #69 merged 2026-04-28 at `be6f183`):** dropped the `mockChats` production seed in `StylistDashboard` (D7) so the chat pane no longer fabricates conversations on first load; passed real authed-stylist initials from the server page so the avatar + chat-bubble fallbacks stop rendering the hardcoded `"SM"` placeholder (D6); switched the dashboard Drafts list from the per-device `localStorage` helper (now deleted) to `GET /api/moodboards?status=draft` and added `DELETE /api/moodboards/[id]` for unsent drafts (D11); permanent `/bag` → `/cart` redirect for Loveable-shaped retailer links (§3.9); verified dashboard create-buttons already hit the real `/stylist/sessions/[id]/{moodboards,styleboards}/new` routes (D13).

**P1 audit batch (PRs #71 / #72 / #73, all merged 2026-04-28):**
- **§3.10 — End-session modal (PR #71 at `de1974c`):** `/sessions/[id]/end-session` is now the Loveable 3-step `PostSessionModal` (Tip → Review → Share) instead of a stacked page. Step 1 collects tip via `computeChipAmounts` chips (15 / 20 / 25) + Custom + Skip; Step 2 collects star rating (required) + 500-char review; Step 3 surfaces the user's `referralCode` as a copyable `${origin}/?ref=<code>` share link. Inline `<Elements>` + `<PaymentElement>` between Step 2 and Step 3 when tip > 0 (existing Stripe path, just inlined). Server-side guards (auth, ownership, already-rated branch, plan price lookup) unchanged; the page route renders the modal via a thin `EndSessionPageClient` wrapper. Persistent `aria-label` on the dialog. Clipboard feature-detected with a "Copy failed" pill fallback for non-secure contexts.
- **D9 — Item Recommendation wire (PR #72 at `6d6f1cd`):** `POST /api/sessions/[id]/messages` now accepts `{ kind: "SINGLE_ITEM" | "TEXT", body, webUrl?, inventoryProductId? }`. SINGLE_ITEM is stylist-only, requires one of `webUrl | inventoryProductId`, and validates http(s) scheme on both client + server (closes `javascript:` / `data:` schemes since `singleItemWebUrl` is rendered into `<a href>`). Dashboard form gains a required Product URL input; `handleSendItem` posts the formatted summary as the message body with the URL on the `singleItemWebUrl` Twilio attribute, optimistically appends to local state (with URL visible in the preview), rolls back on failure, surfaces server errors inline, and clears the error on URL edit.
- **§3.3 + §3.4 — SessionCard redesign + drop hop (PR #73 at `a9547f0`):** `SessionCard` rebuilt to Loveable's contract — large stylist avatar (14×14 mobile / 20×20 desktop), display-serif name + plan badge, latest-message preview (line-clamp-2, status-aware fallback for sessions without messages), relative-time, status-aware action button:
    - `new_board` → "Review Style Board" (high-priority left-accent border)
    - `awaiting_reply` (`PENDING_END_APPROVAL` only — `PENDING_END` stays in chat) → "Approve End"
    - `in_progress` / `booked` → "View Session" / "Continue"
    - `completed` → "Book {firstName} Again" → `/stylists/[profileId]`
    - `closed` (CANCELLED / FROZEN / REASSIGNED) → "View Details" → `/sessions/[id]`
  Card link no longer hops through `/sessions/[id]` (which detail-page-then-redirected); active sessions with a Twilio channel link straight to `/sessions/[id]/chat`. `getSessionsByClient` now includes the latest `Message` row + an unrated-sent `Board` flag + the stylist's `stylistProfile.id`. `tests/e2e/db.ts::createSessionForClient` status union widened to the full `SessionStatus` enum so specs can seed any state.

**P2 audit batch (PRs #76 / #77 / #78 / #80, all merged 2026-04-28):**
- **D8 + D10 + D12 — stylist chrome (PR #76):** Extracted the dashboard's inline top chrome into a shared `<StylistTopBar />` (W logo, Calendar / Bell / Settings, "My bookings", avatar dropdown) mounted via a new `(stylist)/stylist/layout.tsx` so every stylist surface — `/stylist/dashboard`, `/stylist/clients`, `/stylist/payouts`, `/stylist/profile/boards` — renders the same chrome instead of just the dashboard. Dropped the duplicate inline header from the dashboard + the legacy `StylistNav` (Dashboard link + UserButton only). `/stylist/sessions` list collapsed to `redirect("/stylist/dashboard")` since Loveable's stylist surface has no equivalent route. MoodBoardCreator's "Client info" link replaced with a button that opens `<ClientDetailPanel>` as a side-sheet (matching Loveable + the dashboard pattern), not a full-page nav.
- **§3.5 + §3.8 — client surface (PR #77):** `/orders` now uses Loveable's `Tabs` strip (All / Active / Past) with bucket counts. Active = PENDING / ORDERED / SHIPPED; Past = ARRIVED / RETURN_IN_PROCESS / RETURNED. Per-tab empty states. `/feed` cards expose a Save-look heart on the cover image; `listFeedBoards` accepts an optional `userId` and decorates each row with `isFavorited` (single keyed `findMany` against `favorite_boards` per page). Optimistic toggle hits `POST/DELETE /api/favorites/boards`; unauthed clicks redirect to `/sign-in?redirect_url=/feed`.
- **Inline StyleBoard chat-bubble feedback (PR #78):** Three feedback pills (Love / Revise / Not-my-style) render inline on `StyleBoardMessage` for clients on unrated sent boards. Optimistic POST to existing `/api/styleboards/[id]/feedback`; rolls back on HTTP/network failure with an inline error; pills disabled mid-submit so a client can't double-rate (would trip `rateStyleboard`'s P2002 race guard). The "Review look" deep link is preserved alongside.
- **§3.6 — unified /profile (PR #80):** `/closet` collapses to `/profile` (Loveable has no `/closet` route). Same Items / Looks / Collections tabs, headed by `<Avatar>` (initials fallback) + `{firstName}'s Closet` + `Bronze | Gold | Platinum Member` from `User.loyaltyTier`. `/closet` → `/profile` permanent (308) redirect alongside the `/bag` → `/cart` precedent. Internal links (top nav, settings card, collections back-link + delete redirect, robots disallow, route-traversal e2e specs) all point at `/profile` now.

The full audit-driven backlog is now closed; remaining work is founder-gated (see below).

**Client pixel-parity follow-up (PR #85, merged):** `CLIENT-PIXEL-PARITY-TASK.md` re-audit on `smart-spark-craft@8dfd181`. The post-Phase-10 PR backlog (#41/#67/#69/#71/#72/#73/#76/#77/#78/#80/#81) closed the **structural / component-level** parity backlog — every marketing surface has the right sections in the right order with the right shadcn primitives. **It did not deliver pixel parity.** Re-running `<2% delta vs Loveable` against the live Loveable Vite dev server measured 12–48% pixel diffs on every public marketing route except `/match-quiz` (2.6%). Detail in `wishi-style/CLIENT-PARITY-AUDIT.md`. Two distinct drift sources: (1) data drift — Loveable hardcodes 6 stylists / 5 reviews / 4 boards, rebuild reads from Prisma, dev DB has no Loveable-equivalent seed; (2) genuine layout drift on `/`, `/lux`, `/how-it-works`, `/pricing`, `/stylists`, `/gift-cards`. **Closing the remaining gap is a backlog of (a) dev-DB seed alignment + (b) ~6 per-route design-pass PRs**, tracked under `WISHI-REBUILD-PLAN.md`. PR deliverables: `/discover` built; `tests/visual/marketing.spec.ts::waitForReveal` fixed (the existing harness was capturing `Reveal` sections at `opacity: 0` because `fullPage: true` doesn't actually scroll the viewport — every committed darwin baseline on `main` was broken-empty pre-fix; all regenerated under the fix); `tests/visual/client.spec.ts` + `playwright.visual-client.config.ts` for authed empty-state baselines; `marketing.spec.ts` extended with `/discover` `/reviews` `/gift-cards` `/match-quiz`; `.github/workflows/visual-regression.yml` re-introduced as **manual-only** (`workflow_dispatch` trigger only — running it on every PR was the noise that killed PR #61, and that's still the case until the seed alignment lands); `scripts/capture-pair.mjs` + `scripts/diff-all.mjs` retained for future per-route audits.

**Stylist Loveable HEAD parity (this PR — `stylist-pixel-parity`):** `STYLIST-PIXEL-PARITY-TASK.md` re-audit, this time **against Loveable HEAD** (`wishi-reimagined@dfc4652`), not the obsolete pin `13df7f5`. The audit's prior "zero structural drift" verdict only held against `13df7f5`; HEAD had 111 commits / 1840 LOC of unmerged Loveable changes. Tracking model now locked to `wishi-reimagined origin/main` HEAD via auto-memory `feedback_track_loveable_head.md`. Refreshed `wishi-style/STYLIST-PARITY-AUDIT.md` classifies 108 affordances across 5 surfaces (42 HAVE / 24 DRIFT / 33 MISSING / 9 BACKEND-GAP).

Frontend HEAD parity delivered on this branch: dashboard Active bookings + Archive tabs with 24h auto-archive predicate; end-session request button + END_REQUEST chat-card render (kind already in schema); dual CTAs (Create look + End session) when boards delivered; archive reopen affordance (placeholder until BACKEND-GAP-J transition lands); MoodBoardFreestyle component port + Template/Freestyle toggle on MoodBoardCreator; LookCreator filter taxonomy expansion (Style + BodyType for Inspo, Tops subcategories, Availability, Price preset) with active-filters chip bar + per-chip remove + Clear all + canvas keyboard shortcuts (1/2/3); pricePreset wired into search dto; client-side post-filter on `inventoryResults` for Availability + pricePreset; preview note flagging Tops subcats / Inspo Style / Inspo BodyType chips as preview-only until tastegraph exposes facets.

Engineering cleanup that came with the audit: `useChat` no longer hard-fails when Twilio is unreachable — bootstraps from `/api/sessions/[id]/messages` in parallel; surfaces error only when BOTH paths finish AND BOTH fail, clears any pending error when DB succeeds late. API extended to surface `boardId / singleItemInventoryProductId / singleItemWebUrl / authorClerkId` so DB-bootstrapped rows render the inline MoodBoard / StyleBoard / SingleItem cards. `/logout` page (clears Clerk + e2e cookies → homepage; `/logout` added to `isPublicRoute`), `/stylist/bookings` redirects to dashboard per founder direction, LookCreator Shop tab auto-populates inventory on mount.

Visual regression coverage expanded from 4 routes to 11 (`stylist-{dashboard, sessions, clients, workspace-chat, workspace-styleboards, workspace-curated, workspace-cart, moodboard-creator, look-creator, profile-boards, payouts}`) at desktop 1280×800 + Pixel 7 mobile = 22 specs / 32 baselines, with deterministic chat-body fixture (TEXT × 2 + MOODBOARD + STYLEBOARD + SINGLE_ITEM) via new `tests/e2e/db.ts::seedSessionMessages` + `createBoardFixture` helpers; LookCreator visual stubs `/api/products` for replay determinism since tastegraph image URLs lazy-load and vary by run/cache.

Backend gaps logged (Notion roadmap + audit doc) for the next sweep: BACKEND-GAP-E/F/G/H (tastegraph filter facets — Style / BodyType / Retailer / Availability / Tops subcats), BACKEND-GAP-I (LookCreator closet sub-tabs Cart/Purchased — frontend-only wire-up over existing data), BACKEND-GAP-J (reopen archived session transition + API). Two earlier deferred items also remain on the roadmap: `/stylist/dressing-room` destination route, `inspiration_photos` table seed for MoodBoardCreator. The audit's BACKEND-GAP-A/B/C/D entries (END_SESSION_REQUEST kind, Session.endRequestedAt, archival predicate, drafts list completeness) all turned out to be already-shipped or client-side-fixable.
...
1. Split `/bookings/new` into `/select-plan` + `/session-checkout` — or keep single-page? (`/checkout` was resolved 2026-04-29 — PR #81 ships native Stripe Elements form per Loveable contract, replacing the prior Hosted Checkout placeholder.)
